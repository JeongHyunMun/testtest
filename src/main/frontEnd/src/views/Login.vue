...
async function login() {
  ...
  axios.post('/hyeon/login', formData, {
  })
  .then(res => {
    if (res.data.user != null) {
      loginUser.login(res.data.user.user_name, res.data.user.user_no);
      router.push("/home")
    } else {
      alert("로그인 실패!" + res.data + " / " + res.data.user + " / " + res.data.accessToken);
    }
...
async function signUp() {
  if (!id.value || !pw.value || !passwordCheck.value || !signUpName.value) {
    alert("모든 값을 입력하세요.");
    return;
  }
  ...
  formData.append("user_password", pw.value)
  formData.append("user_name", signUpName.value)
  formData.append("role_name", "USER")
  
  axios.post('/hyeon/signup', formData, {
  })
  .then(res => {
    if (res.responseCode.code == "0000") {
      alert("회원가입 성공" + res );
      window.location.href = "/";
    } else {
      alert("회원가입 실패" + res.responseCode.code);
    }
  ...
  <div class="container" :class="{ 'right-panel-active': isSignUp }">
    <div class="form-container sign-up-container">
      <form @submit.prevent="signUp">
        ...
        <input type="password" v-model="passwordCheck" placeholder="Password Check" />
